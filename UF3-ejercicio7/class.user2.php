...
class User {
        ...
        public function Save() {
                if (isset($this->ID)) {
                        ...
                        $objStatement->execute();
                        $this->log->logMessage("Se ha ACTUALIZADO el usuario con id " . $this->ID . "\n", Logger::DEBUG);
                } else {
                        $strValueList = "";
                        ...
                        $this->log->logMessage("Se ha INSERTADO el usuario con id " . $this->ID . "\n", Logger::DEBUG);
                }
        }

        public function MarkForDeletion() {
                $this->blForDeletion = true;
                $this->log->logMessage("Se ha marcado para borrar un usuario \n", Logger::WARNING);
        }
        ...
        public function __destruct() {
                if (isset($this->ID)) {
                        if ($this->blForDeletion == true) {
                                ...
                                $objStatement->execute();
                                $this->log->logMessage("Se ha BORRADO el usuario con id: " . $this->ID . "\n", Logger::CRITICAL);
                        };
                }
        }
        ...
        public function __call($strFunction, $arArguments) {
                ...
                switch ($strMethodType) {
                        case "set":
                                $this->log->logMessage("Se ha creado una función set\n", Logger::INFO);
                                return($this->SetAccessor
                                      ($strMethodMember, $arArguments[0]));
                                break;
                        case "get":
                                $this->log->logMessage("Se ha creado una función get\n", Logger::INFO);
